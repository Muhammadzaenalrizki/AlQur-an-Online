<template >
  <div class="p-3 bg-white shadow flex justify-between lg:px-36">
    <div>
      <span class="text-green-400 text-2xl font-medium">Al Qur'an Online</span>
    </div>
    <div>
      <span>DarkMode</span>
    </div>
  </div>
  <div class="flex justify-center">
    <span class="text-gray-400 text-3xl mt-10">Baca Al Qur'an</span>
  </div>
  <div class="flex justify-center">
    <div>
      <input
        type="text"
        placeholder="search surah"
        class="text-gray-400 w-96 focus:outline-none text-xl shadow-md p-2 border-2 rounded-full border-green-300 mt-3"
      />
      <button class="z-50 relative right-12 top-2">
        <svg
          width="28"
          height="28"
          viewBox="0 0 24 24"
          fill="#ffffff "
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M10 18C11.775 17.9996 13.4988 17.4054 14.897 16.312L19.293 20.708L20.707 19.294L16.311 14.898C17.405 13.4997 17.9996 11.7754 18 10C18 5.589 14.411 2 10 2C5.589 2 2 5.589 2 10C2 14.411 5.589 18 10 18ZM10 4C13.309 4 16 6.691 16 10C16 13.309 13.309 16 10 16C6.691 16 4 13.309 4 10C4 6.691 6.691 4 10 4Z"
            fill="#34D399"
          />
          <path
            d="M11.4118 8.58599C11.7908 8.96599 11.9998 9.46799 11.9998 9.99999H13.9998C14.0007 9.47442 13.8974 8.95389 13.6959 8.46848C13.4944 7.98307 13.1987 7.54242 12.8258 7.17199C11.3118 5.65999 8.68683 5.65999 7.17383 7.17199L8.58583 8.58799C9.34583 7.82999 10.6558 7.83199 11.4118 8.58599Z"
            fill="#34D399"
          />
        </svg>
      </button>
    </div>
  </div>
  <div class="lg:px-36">
    <div class="flex flex-wrap lg:mt-2">
      <div
        class="bg-white flex shadow border-2 border-gray-200 p-3 lg:w-72 lg:my-2 lg:mx-2"
      >
        <div class="w-1/2 flex justify-center">
          <div
            class="mr-4 bg-green-100 text-center h-7 w-14 text- rounded-full"
          >
            <span class="mt-2 text-lg"> 1 </span>
          </div>
          <div class="">
            <span class="text-lg font-medium"> Al-Fatihah </span>
            <span class="text-md text-gray-500"> Pembukaan </span>
          </div>
        </div>
        <div class="w-1/2">
          <span class="ml-16 text-2xl text-green-400"> ٱلْفَاتِحَةِ </span>
        </div>
      </div>
    </div>

    <footer class="text-center">&copy;zaenal</footer>
  </div>
</template>
<script>
export default {
  name: "App",
  components: {},
};
</script>


